...
const imbService = require('../services/imbPeopleGroupsService');
...
router.post('/import', auth, isMissionary, upload.single('file'), async (req, res) => {
  try {
    ...
    const result = await imbService.importFromCSV(req.file.buffer);
    res.json(result);
  } catch (err) {
    console.error('[IMB] import error:', err);
...
router.get('/people-groups', auth, validatePagination, validateCountryCode, async (req, res) => {
  try {
    ...
    const result = await imbService.getAllPeopleGroups(options);
    res.json({ success: true, ...result });
  } catch (err) {
    console.error('[IMB] people-groups error:', err);
    res.status(500).json({ success: false, error: err.message });
  }
});
...
router.get('/people-groups/unreached', auth, validatePagination, async (req, res) => {
  try {
    ...
    console.error('[IMB] unreached error:', err);
    res.status(500).json({ success: false, error: err.message });
  }
});
...
router.get('/status', auth, async (req, res) => {
  try {
    const statistics = await imbService.getSyncStatus();
    res.json({ success: true, statistics });
  } catch (err) {
    console.error('[IMB] status error:', err);
    res.status(500).json({ success: false, error: err.message });
  }
});
...
router.delete('/clear', auth, isMissionary, async (req, res) => {
  try {
    const result = await imbService.clearAllData();
    res.json({ success: true, message: 'IMB / PeopleGroups.org data cleared', ...result });
  } catch (err) {
    console.error('[IMB] clear error:', err);
    res.status(500).json({ success: false, error: err.message });
  }
});
