...
class ImbPeopleGroupsService {
  ...
  async importFromCSV(buffer) {
    ...
    for (const row of rows) {
      try {
        // flexible column lookup – PeopleGroups.org common headers
        const name = sanitizeString(
          pick(row, 'PeopNameInCountry', 'PeopleName', 'Name', 'name', 'people_name', 'peoplename')
        );
        if (!name) continue;
        ...
        let countryCode = sanitizeString(
          pick(row, 'ROG3', 'CountryCode', 'country_code', 'countrycode', 'ISO2', 'iso2')
        ).toUpperCase();
        ...
        const language = sanitizeString(
          pick(row, 'PrimaryLanguageName', 'Language', 'language', 'lang')
        );
        const religion = sanitizeString(
          pick(row, 'PrimaryReligion', 'Religion', 'religion')
        );
        const population = parseIntVal(
          pick(row, 'Population', 'population', 'pop', 'Pop')
        );
        const region = sanitizeString(
          pick(row, 'Region', 'region', 'RegionName', 'regionname')
        );
        ...
        if (result) {
          // distinguish insert vs update by checking createdAt vs updatedAt proximity
...
module.exports = new ImbPeopleGroupsService();
