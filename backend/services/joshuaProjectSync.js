...
function transformJPRecord(record, adminId) {
  ...
  let name = (record.PeopNameInCountry || record.PeopNameAcrossCountries || '').trim();
  // Pad single-character names (e.g. "E" people group in China) so they pass the >=2 char validator
  if (name.length === 1) {
    name = countryName ? `${name} (${countryName})` : `${name} people`;
  }
